...
function formatDate(iso: string) {
  return new Date(iso.replace("Z","")).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" });
}
function formatTime(iso: string) {
  return new Date(iso.replace("Z","")).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true });
}
function timeUntil(iso: string) {
  const diff = new Date(iso.replace("Z","")).getTime() - Date.now();
                    ...
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                          ...
                          {/* WhatsApp */}
                          <a href={`https://wa.me/?text=${encodeURIComponent(`Hi ${meeting.client_name}, your meeting is scheduled on ${formatDate(meeting.start_time)} at ${formatTime(meeting.start_time)}. Join here: ${meeting.meet_link}`)}`}
