...
// Map JS getDay() (0=Sun) to DB day name
...
export default function BookingPage() {
  ...
  useEffect(() => {
    fetch("/api/meetings/settings")
      .then(r => r.json())
      .then(data => setDaySettings(data))
      .catch(() => {/* silently fallback — all days will be treated as disabled */})
      .finally(() => setSettingsLoading(false));
  }, []);

  const getDaySetting = (year: number, month: number, day: number): DaySetting | undefined => {
    const jsDay = new Date(year, month, day).getDay();
    const name  = JS_DAY_TO_NAME[jsDay];
    return daySettings.find(d => d.day === name);
  };
  ...
  const isDisabled = (day: number) => {
    ...
    const setting = getDaySetting(currentYear, currentMonth, day);
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#ffffff", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cal-day:hover:not(.disabled) { background: #e8f2f9; }
        ...
        .input-field {
          ...
          outline: none; transition: border 0.15s; font-family: inherit; background: #fff;
        }
        ...
        .modal-overlay {
          ...
          animation: fadeIn 0.2s ease; padding: 0;
        }
        ...
        .nav-btn {
          ...
          justify-content: center; color: #1b3e5c; transition: all 0.15s; font-size: 16px;
        }
        ...
        .page-header { border-bottom: 1px solid #eef2f6; padding: 16px 20px; display: flex; align-items: center; gap: 12px; }
        @media (min-width: 640px) { .page-header { padding: 20px 40px; } }

        .hero { padding: 32px 20px 24px; text-align: center; }
        @media (min-width: 640px) { .hero { padding: 48px 40px 36px; } }
        .hero h1 { font-family: 'Playfair Display', serif; font-size: 26px; font-weight: 700; color: #1b3e5c; line-height: 1.2; margin-bottom: 12px; }
        @media (min-width: 640px) { .hero h1 { font-size: 36px; } }

        .main-grid { max-width: 960px; margin: 0 auto; padding: 0 16px 60px; display: flex; flex-direction: column; gap: 20px; }
        @media (min-width: 768px) { .main-grid { padding: 0 40px 60px; display: grid; grid-template-columns: 1fr 1fr; gap: 32px; } }

        .panel { background: #fff; border: 1px solid #eef2f6; border-radius: 16px; padding: 20px; box-shadow: 0 4px 24px rgba(27,62,92,0.06); }
        @media (min-width: 640px) { .panel { padding: 28px; } }

        .slots-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; max-height: 220px; overflow-y: auto; }
        @media (min-width: 480px) { .slots-grid { grid-template-columns: repeat(4, 1fr); } }
        @media (min-width: 768px) { .slots-grid { grid-template-columns: 1fr 1fr; } }

        .right-col { display: flex; flex-direction: column; gap: 20px; }

        /* Disabled day legend dot */
        .legend-dot { width: 10px; height: 10px; border-radius: 50%; display: inline-block; }

        @media (max-width: 640px) {
          .page-header { flex-wrap: wrap; }
          .hero h1 { font-size: 24px; }
          .main-grid { padding: 0 14px 48px; }
          .panel { padding: 18px; }
        }
        @media (max-width: 480px) {
          .cal-day { width: 32px; height: 32px; font-size: 12px; }
          .slots-grid { grid-template-columns: repeat(2, 1fr); }
          .slot-btn { font-size: 12px; padding: 9px 6px; }
        }
      `}</style>
      ...
      <div className="hero" style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        <p style={{ color: "#7a8fa0", fontSize: 14, maxWidth: 480, margin: "0 auto" }}>
          Select a date and time that works for you. We will send a Google Meet link straight to your inbox.
        </p>
      </div>
      ...
      <div className="main-grid">
        ...
        <div className="panel" style={{ alignSelf: "start" }}>
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div key={day} style={{ display: "flex", justifyContent: "center" }}>
                  <button
                    className={`cal-day ${selected ? "selected" : ""} ${isToday && !selected ? "today" : ""} ${disabled ? "disabled" : ""}`}
                    onClick={() => handleDateClick(day)}
                    title={disabled && !isPast(day) ? "Not available" : undefined}
                  >
                    {day}
                  </button>
                </div>
          ...
          <div style={{ marginTop: 20, paddingTop: 16, borderTop: "1px solid #eef2f6" }}>
            {settingsLoading ? (
              <p style={{ fontSize: 12, color: "#c0c8d0" }}>Loading availability...</p>
            ) : (
              <div style={{ display: "flex", gap: 16, fontSize: 12, color: "#7a8fa0", flexWrap: "wrap" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  <span className="legend-dot" style={{ background: "#2b7aab" }} />Available
                </span>
                <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  <span className="legend-dot" style={{ background: "#e5e7eb" }} />Unavailable
                </span>
              </div>
            )}
          </div>
              ...
              <>
                ...
                {slots.length === 0 ? (
                  ...
                  <div className="slots-grid">
                    {slots.map((slot, i) => (
                      <button key={i} className={`slot-btn ${selectedSlot === slot ? "selected" : ""}`} onClick={() => setSelectedSlot(slot)}>
          ...
          {selectedSlot && (
            <div className="panel" style={{ animation: "slideUp 0.2s ease" }}>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <input className="input-field" placeholder="John Smith" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
              </div>
              ...
              <div style={{ marginBottom: 12 }}>
                <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: "#1b3e5c", marginBottom: 6 }}>
                  Contact Number <span style={{ color: "#7a8fa0", fontWeight: 400 }}>(optional)</span>
                </label>
                <input className="input-field" type="number" placeholder="Contact number" value={form.contact_number} onChange={e => setForm(f => ({ ...f, contact_number: e.target.value }))} />
              </div>

              <div style={{ marginBottom: 12 }}>
                <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: "#1b3e5c", marginBottom: 6 }}>Purpose of Meeting</label>
                <select className="select-field" value={form.purpose} onChange={e => setForm(f => ({ ...f, purpose: e.target.value, userInput: "" }))}>
              ...
              {form.purpose && (
                <div style={{ marginBottom: 20, animation: "slideUp 0.2s ease" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                    <label style={{ fontSize: 12, fontWeight: 500, color: "#1b3e5c" }}>{PURPOSE_LABELS[form.purpose]}</label>
                    <span className="ai-badge">AI Enhanced</span>
                  </div>
                  <textarea className="textarea-field" placeholder={PURPOSE_PLACEHOLDERS[form.purpose]} value={form.userInput} onChange={e => setForm(f => ({ ...f, userInput: e.target.value }))} />
              ...
              <button className="book-btn" onClick={handleBook} disabled={!canBook}>
        ...
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div style={{ width: 56, height: 56, background: "#e8f7ee", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
              <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#2ecc71" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
            </div>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, color: "#1b3e5c", marginBottom: 8 }}>You are Booked!</h2>
